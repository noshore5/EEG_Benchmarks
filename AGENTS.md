...
repo is worked on from multiple agent shells (Claude, Grok, others) that
don't share context with each other, so `CONTEXT.md` is what keeps a new
session from re-deriving things a previous one already figured out.
