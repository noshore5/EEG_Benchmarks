...
from BCI import run_pipelines as _wct_run
...
from BCI.moabb_pipelines.msc_evidence_gnn import (
...
# run_pipelines.py no longer keeps a display-name-keyed PIPELINE_BUILDERS/
# PIPELINE_PARAM_GRIDS pair (MSC-Evidence-GNN isn't one of its 6 pipelines),
# so this script builds its own local copies from PIPELINE_SPECS instead.
PIPELINE_BUILDERS = {
    spec["display"]: (lambda spec=spec: spec["cls"](**spec["params"]))
    for spec in _wct_run.PIPELINE_SPECS.values()
}
PIPELINE_BUILDERS["MSC-Evidence-GNN"] = _make_msc_evidence_gnn
DEFAULT_PIPELINES = ["MSC-Evidence-GNN"]
PIPELINE_PARAM_GRIDS = {name: {} for name in PIPELINE_BUILDERS}
...
# The following four helpers used to live on run_wct_gnn.py and were reused
# from there; run_pipelines.py (its successor) dropped the grid-search/inner-
# CV-grouping machinery they support, since none of its 6 pipelines actually
# use multi-value grids anymore. Kept here, local to this MSC ablation
# script, since MSC-Evidence-GNN's own grid still does.
def _build_pipeline_runs(pipeline_names, inner_group_mode, global_hyperparam_fit_mode, cross_subject=False):
    if inner_group_mode == "none":
        ...
        raise ValueError(f"Unsupported inner_group_mode='{inner_group_mode}'.")

    eval_modes = _wct_run._resolve_eval_modes(global_hyperparam_fit_mode, cross_subject)
    deduped_pipelines = list(dict.fromkeys(pipeline_names))
    runs = []
    for name in deduped_pipelines:
        for eval_mode in eval_modes:
            for inner_group in inner_groups:
                label = f"{name} [eval={eval_mode}] [inner_group={inner_group or 'None'}]"
                runs.append(
                    {"base_name": name, "eval_mode": eval_mode, "inner_group": inner_group, "label": label}
                )
    return runs
...
def _apply_fixed_param_overrides(pipelines, run_param_grid, fixed_overrides):
    ...
    return {
        label: {name: values for name, values in grid.items() if name not in fixed_overrides}
...
def _print_inner_results(inner):
    ...
    if inner.empty:
        log_event(log, EventCategory.FINAL_RESULTS, "No inner cv_results_ collected.")
        return
    summary_cols = [
        c
        for c in [
            "eval_type", "dataset", "subject", "session", "pipeline",
            "outer_fold", "params", "mean_test_score", "mean_train_score",
            "rank_test_score",
        ]
        if c in inner.columns
    ]
    log_event(log, EventCategory.FINAL_RESULTS, inner[summary_cols].to_string(index=False))
    split_cols = [
        c
        for c in [
            "subject", "session", "pipeline", "outer_fold", "params",
    ...
    if split_cols != summary_cols:
        ...
        log_event(log, EventCategory.FINAL_RESULTS, inner[split_cols].to_string(index=False))
