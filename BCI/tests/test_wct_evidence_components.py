...
from BCI.moabb_pipelines.wct_phase_gnn_classifier import (
...
from BCI.nn_components import CategoricalGateConfig, SelectPath
...
def test_run_pipelines_wct_evidence_gnn_config_smoke() -> None:
    from BCI import run_pipelines

    spec = run_pipelines.PIPELINE_SPECS["wct_evidence_gnn"]
    estimator = spec["cls"](**spec["params"])
    ...
    assert estimator.window_compute_mode == "chunked"
    assert estimator.max_windows_per_chunk == 1
    assert estimator.last_batch_min_ratio == 0.5
    assert estimator.selector_alpha_val_update_rate == 0.5
    assert core._resolve_window_compute_mode() == "chunked"
...
def test_run_pipelines_writes_human_readable_result_companions(tmp_path) -> None:
    ...
    run_pipelines._write_group_artifacts(
        ...
        subjects=[1],
        eval_mode="cross",
        effective_params={"test-pipeline": {"batch_size": 8}},
        data_root=tmp_path,
...
def test_run_pipelines_managed_run_id_comes_from_environment(monkeypatch) -> None:
    from BCI import run_pipelines

    monkeypatch.setenv("LOCAL_ORCHESTRATION_EXECUTION_ID", "managed-run-1")

    assert run_pipelines._safe_run_id(None) == "managed-run-1"
    with pytest.raises(ValueError, match="Run ID"):
        run_pipelines._safe_run_id("invalid run id")
...
def test_noise_channel_std_uses_training_split_after_resampling(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "BCI.moabb_pipelines.xwt_phase_gnn_classifier."
