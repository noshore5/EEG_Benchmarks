...
class EEGNetClassifier(ChannelSubsetMixin, TorchEEGClassifier):
    ...
    def __init__(
        ...
        # Restrict which channels are used, by integer index (or by name
        # if self.channel_names_ has been populated before fit()). See
        # ChannelSubsetMixin in common.py.
    ...
    def _prepare_features(self, X: np.ndarray, *, fit: bool, train_idx=None):
        # Slice channels first, before normalization/model sizing touches
        # X, so everything downstream (z-score stats, n_channels inference)
        # only ever sees the subset.
