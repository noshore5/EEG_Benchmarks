...
try:
    import fcntl  # POSIX-only; run_ledger append lock degrades gracefully without it
...
try:
    from Epilepsy.experiment_logging import (
        EventCategory,
        get_console_policy,
...
def resolve_coherence_utils():
    ...
    if configured_root:
        candidate_roots = [Path(configured_root).expanduser().resolve()]
    else:
        candidate_roots = [
            repo_root / "Coherent_Multiplex",
            # This repo vendors a partial Coherent_Multiplex copy (just
            # coherence_utils.py, no brain_utils/fft_utils/signal_utils)
            # directly under utils/ instead of a nested Coherent_Multiplex/
            # subfolder -- fall back to the repo root itself so this resolves
            # without WCT_COHERENT_MULTIPLEX_ROOT being set.
            repo_root,
        ]
    coherent_root = next(
        (root for root in candidate_roots if (root / "utils" / "coherence_utils.py").is_file()),
        candidate_roots[0],
    )
...
# --- Cross-run results ledger -----------------------------------------
#
# One durable, append-only CSV recording every completed outer-CV row across
# every run_pipelines.py invocation, so "did I already test subject N at
# phase_threshold_deg=X" is a pandas filter instead of a multi-file log
# search. Built 2026-08-08 after exactly that question required grepping
# ~120 run directories under ~/mne_data -- see run_pipelines.py's
# `_write_group_artifacts` docstring for the underlying problem this solves:
# per-run-id `summary_<group_id>.md`/`scores_<group_id>.csv` files overwrite
# on every reuse of a run-id (e.g. "canonical-sparse" reused ~47 times for
# both the actual canonical baseline and one-off scratch tests), so only the
# *last* run under any given run-id survives there. The raw timestamped
# experiment_*.log files are the only durable record prior to this ledger.
...
RUN_LEDGER_CONFIG_KEYS = [
    ...
    "surrogate_count",
    "surrogate_seed",
    "cluster_significance_percentile",
    "epochs",
    "batch_size",
    "seed",
    "highest",
...
RUN_LEDGER_COLUMNS = (
    ...
    + RUN_LEDGER_CONFIG_KEYS
    + ["experiment_log_path", "effective_params_json", "fixed_overrides_json"]
)
...
def default_run_ledger_path() -> Path:
    ...
    return Path(configured).expanduser() / "run_ledger.csv"


def run_ledger_rows_from_group(
    *,
    run_id: str,
    group_id: str,
    eval_mode: str,
    ...
    """Build one run_ledger row per outer-CV (subject, session, pipeline) row
    in `group_results`, joining in that pipeline's effective config
    (`RUN_LEDGER_CONFIG_KEYS`, pulled from `effective_params[pipeline]`) so
    the ledger is queryable by hyperparameter without re-parsing logs. Pass
    the returned rows to `append_run_ledger_rows`."""
    if logged_at is None:
        logged_at = datetime.now(timezone.utc).isoformat()
    fixed_overrides_json = json.dumps(fixed_overrides or {}, default=str)
    rows = []
    for record in group_results.to_dict("records"):
        pipeline = record.get("pipeline")
        params = (effective_params or {}).get(pipeline, {}) or {}
        ledger_row = {
            "logged_at": logged_at,
            "run_id": run_id,
            "group_id": group_id,
            ...
            "experiment_log_path": (
                str(experiment_log_path) if experiment_log_path else None
            ),
            "effective_params_json": json.dumps(params, default=str),
            "fixed_overrides_json": fixed_overrides_json,
        }
        for key in RUN_LEDGER_CONFIG_KEYS:
            ledger_row[key] = params.get(key)
        rows.append(ledger_row)
    return rows


def append_run_ledger_rows(rows: list[dict], ledger_path: Path | None = None) -> Path:
    """Append rows to the persistent cross-run results ledger (default
    `~/mne_data/run_ledger.csv`). Uses an flock-based advisory lock on a
    sibling `.lock` file so concurrent run_pipelines.py invocations can't
    interleave writes into a torn CSV -- the same failure mode already
    documented (and NOT fixed) for the per-run `scores_<group_id>.csv` files
    in run_pipelines.py's `_write_group_artifacts`. Best-effort only: on
    platforms without `fcntl` (e.g. Windows), the append proceeds unlocked."""
    if not rows:
        return ledger_path or default_run_ledger_path()
    if ledger_path is None:
        ledger_path = default_run_ledger_path()
    ...
    try:
        ...
        frame = pd.DataFrame(rows).reindex(columns=RUN_LEDGER_COLUMNS)
        file_exists = ledger_path.exists() and ledger_path.stat().st_size > 0
        frame.to_csv(ledger_path, mode="a", header=not file_exists, index=False)
    finally:
        if fcntl is not None:
...
class ChannelSubsetMixin:
    ...
    ``self.channel_names_`` (list[str] | None, only needed if
    channel_subset is given as names) in ``__init__``, then call
    ``self._apply_channel_subset(X)`` on the raw (n_trials, n_channels,
    n_time) array before any other channel-shape-dependent processing.
    """
...
class TorchEEGClassifier(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y, validation_groups: np.ndarray | None = None, metadata=None):
        ...
        # 2026-08-10: explicit generator, seeded independently of torch's
        # global RNG state -- without this, shuffle=True with no `generator`
        # draws its per-epoch batch order from torch's GLOBAL RNG at
        # iteration time, which is AFTER self.model_ is built below (line
        # ~1499). That means the actual batch order every epoch trains on
        # depends on exactly how many random draws model construction
        # consumed -- add, remove, or reorder ANY nn.Module in __init__
        # (even one that's dead code at runtime, e.g. a new head gated off
        # by a constructor flag) and every subsequent epoch sees a different
        # batch sequence under the "same" self.seed. Confirmed directly on
        # Sparse-Evidence-GNN: four identical-config (subject=2, seed=44,
        # epochs=75) runs at different points in that file's __init__
        # history gave 1test scores ranging 0.43-0.56 -- see
        # [[sparse-evidence-gnn-event-gate-init-shift-bug]] in project
        # memory for the investigation. Tying shuffle order to self.seed
        # directly (independent of however many params get built in
        # between) makes "same seed" runs comparable across architecture
        # edits, which they were not before this fix.
